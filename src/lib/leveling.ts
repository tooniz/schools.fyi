import { EvidenceSchema, OffsetRuleSchema, ProgramSchema, gradeLevels, levelingSubjects } from "./schema";
import type { ConfidenceTier, Evidence, LevelingSubject, OffsetRule, Program } from "./schema";
...
const bandKey = (rule: OffsetRule) => `${rule.programId}/${rule.subject}`;
...
export function validateLeveling(input: { programs: unknown[]; evidence: unknown[]; rules: unknown[] }): LevelingDataset {
  ...
  for (const program of programs) {
    for (const [key, claimed] of covered) {
      if (!key.startsWith(`${program.id}/`)) continue;
  ...
  const dataset = { programs, evidence, rules };
  for (const subject of levelingSubjects) {
    for (const column of buildLeveling(dataset, subject, programs.map(({ id }) => id))) {
      column.cells.reduce<LevelingCell | null>((previous, cell) => {
        if (previous && cell.progressStart < previous.progressStart - 1e-9) {
          throw new Error(`${column.program.id}/${subject}: ${cell.label} starts before ${previous.label} on the progress axis`);
        }
        return cell;
      }, null);
    }
...
export function buildLeveling(dataset: LevelingDataset, subject: LevelingSubject, programIds: string[]): LevelingColumn[] {
  return programIds.flatMap((programId) => {
    ...
    const cells = program.gradeLabels.flatMap<LevelingCell>((label, gradeIndex) => {
      const rule = subjectRules.find(({ fromGradeIndex, toGradeIndex }) => gradeIndex >= fromGradeIndex && gradeIndex <= toGradeIndex);
    ...
    return [{ program, cells, headlineOffset }];
  });
}
 ...
 * position its offset actually claims. A level is stretched backwards to meet
 * its predecessor so a program's ladder reads as a continuous column rather
 * than a set of floating boxes. And where one program puts two levels on the
 * same stretch of the axis, that stretch is given proportionally more height
 * and every other program's level across it grows to match, so converging
 * levels sit side by side in time without being drawn on top of each other.
 */
export function layoutMatrix(columns: LevelingColumn[]): MatrixLayout {
  const placements = columns.map((column) => {
    ...
    return column.cells.map((cell) => {
      const stretched = previousEnd !== null && cell.progressStart > previousEnd + EPS;
...
export function offsetLabel(offsetYears: number): string {
  if (offsetYears === 0) return "On Ontario pace";
